...
var elyes =
    "je m'apelle elyes et j'aime les var et dynamiter git et les joint de culasse a la masse";
...
const sctnSelect = createMarkup("section", "", container, [
    { class: "d-flex flex-row justify-content-around my-5" },
]);
const sctnRecipes = createMarkup("section", "", container, [
    { class: "row", id: "section-card-recipes" },
]);
 ...
 * Insertion du Titre
 */
const Title = createMarkup("h1", "Master Cook", sctnTitle, [
    { class: "text-center" },
]);
/**
 * creation select show Recipes
 */
const divContentsctnSelectShowRecipes = createMarkup("div", "", sctnSelect, [
    { class: "d-flex justify-content-center flex-column" },
]);
const labelShowRecipes = createMarkup(
    "label",
    "Choisir une recette",
    divContentsctnSelectShowRecipes,
    [{ class: "form-control" }]
);
const selectShowRecipes = createMarkup(
    "select",
    "Choisir une recette",
    divContentsctnSelectShowRecipes,
    [{ class: "form-select", id: "select-all-recipes" }]
);
/**
 * creation select show per country
 */
const divContentsctnSelectPerCountry = createMarkup("div", "", sctnSelect, [
    { class: "d-flex justify-content-center flex-column" },
]);
const labelPerCountry = createMarkup(
    "label",
    "Selectionner un pays",
    divContentsctnSelectPerCountry,
    [{ class: "form-control" }]
);
const selectPerCountry = createMarkup(
    "select",
    "Selectionner un pays",
    divContentsctnSelectPerCountry,
    [{ class: "form-select", id: "select-per-country" }]
);
/**
 * creation select show per ingredient
 */
const divContentsctnSelectPerIngredient = createMarkup("div", "", sctnSelect, [
    { class: "d-flex justify-content-center flex-column" },
]);
const labelPerIngredient = createMarkup(
    "label",
    "Selectionner un Ingredient",
    divContentsctnSelectPerIngredient,
    [{ class: "form-control" }]
);
const selectPerIngrendient = createMarkup(
    "select",
    "Selectionner un Ingredient",
    divContentsctnSelectPerIngredient,
    [{ class: "form-select", id: "select-per-ingredient" }]
);
/**
 * creation button creation recette et listener sur le bouton
 */
const divContentsctnBtnCreationRecipes = createMarkup("div", "", sctnSelect, [
    { class: "d-flex justify-content-center flex-column" },
]);
const btnCreateRecipes = createMarkup(
    "button",
    "Créé une recette",
    divContentsctnBtnCreationRecipes,
    [{ class: "btn btn-success", id: "btnCreateRecipes" }]
);
...
function run() {
    document
        .getElementById("btnCreateRecipes")
        .addEventListener("click", showModalCreateRecipes);
    /**
    ...
    document
        ...
        .addEventListener("click", addInputIngredient);
    document
        .getElementById("btn-record-edit")
        .addEventListener("click", recordRecipe);
    ...
    selectShowRecipes.addEventListener("change", (event) => {
        ...
        let filteredRecipes = arrayDataRecipes.filter(
...
function addInputIngredient() {
    ...
    const divCreateRecipesModalInput = createMarkup(
        "div",
        "",
        divCreateRecipesModal,
        [{ class: "d-flex flex-row value-ingredient-create" }]
    );
    createMarkup("input", "", divCreateRecipesModalInput, [
        { class: "form-control my-1 mx-1 ", placeholder: "ingredient" },
    ]);
    createMarkup("input", "", divCreateRecipesModalInput, [
        { class: "my-1 mx-1", type: "number" },
    ]);
    const selectCreateRecipes = createMarkup(
        "select",
        "Choisir une uniter",
        divCreateRecipesModalInput,
        [{ class: "unit-select unit form-select my-1 mx-1" }]
    );
...
async function recordRecipe() {
    ...
    const tabValueIngredient = document.getElementsByClassName(
        "value-ingredient-create"
    );
    ...
    if (tabValueIngredient.length > 0) {
        ...
        try {
            ...
            const response = await fetch(
                `${endPoint}/recipes?gastronomy=${gastronomy}`,
                {
                    method: "post",
                    headers: {
...
async function getRecipesByIngredientName(selectedIngredient) {
    try {
        const response = await fetch(
            `${endPoint}/recipes/?ingredient=${selectedIngredient}`,
            {
                method: "get",
                headers: {
                    "Content-Type": "application/json",
...
async function getRecipesByGastronomyName(selectedCountry) {
    try {
        const response = await fetch(
            `${endPoint}/recipes/?gastronomy=${selectedCountry}`,
            {
                method: "get",
                headers: {
                    "Content-Type": "application/json",
...
function createRecipesCard(dataRecipes) {
    ...
    dataRecipes.forEach((recipe, index) => {
        // Parcours de chaque recette dans les données
        let ingredientsHtml = "";
    ...
    for (let i = 0; i < btnEditCards.length; i++) {
        // Parcours de chaque bouton d'édition
        btnEditCards[i].addEventListener("click", (event) => {
            // Ajout d'un écouteur d'événement au clic sur le bouton d'édition
    ...
    for (let i = 0; i < btnSupprCard.length; i++) {
        // Parcours de chaque bouton d'édition
        btnSupprCard[i].addEventListener("click", (event) => {
            // Ajout d'un écouteur d'événement au clic sur le bouton d'édition
            const card = event.target.closest(".card"); // Récupération de la carte parente du bouton
            if (card) {
...
function updateCardData(recipeId, updatedRecipe) {
    ...
    if (!recipeCard) {
        console.error(
            `La carte de recette avec l'ID ${recipeId} n'a pas été trouvée.`
        );
        return;
    }
    ...
    updatedRecipe.ingredients.forEach((updatedIngredient, index) => {
        ...
        if (ingredientElement) {
            ...
            const quantityElement = ingredientElement.querySelector(
                ".ingredient-quantity"
            );
...
function fillSelectPerIngredient(dataRecipes) {
    const selectPerIngredient = document.getElementById("select-per-ingredient");
    let htmlContent =
        "<option value='empty' selected>Choisir un Ingredient</option>";
...
function fillSelectUnitCreateRecipes(dataRecipes) {
    const selectUnitCreateRecipes = document.getElementById(
        "select-unit-create-recipes"
    );
